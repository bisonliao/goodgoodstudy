...
[整理coco数据的python脚本](code/yolo/gen_bbox.py)

还是前面整理的火车的图片作为训练样本，运行10来个小时140多个epoch后，loss收敛了3个数量级，准确度90%+，但召回率比较低，只有10%（置信度阈值设置为0.5）。怎么统计的呢？

1. 每个图片49个cell，如果某个cell标注说没有物体、预测也说没有物体，那么算模型准确预测一次；如果某个cell标注说有火车，预测也说有火车，那么算模型准确预测一次。准确预测的次数除以49就是准确率，进一步扩大到很多图片
2. 如果标注说某个cell有火车，预测也说有火车，算成功召回一次。成功召回的总次数比标注声称有火车的次数，即召回率

用一些测试样例简单测试，可以看出模型很保守，大多数情况都判断为没有火车，所以准确率很高，召回率很低：

![结果图片](img/yolo/result5.jpg)

怎么改进呢？做了两个措施，发现有效果：

1. 增加大量的“没有火车”的负样本图片，放到训练集和验证集合里
2. 调整损失函数，对于标注有“火车”但预测说没有“火车”这种导致召回率偏低的情况加大“惩罚”，系数从1改为10；

训练25个epoch后loss不怎么收敛了。验证集上得到准确率为98%，召回率为44.6%。但从一组观察的样本图片来看，召回率不止44.6%， 都召回了，有一张误判：

![结果图片](img/yolo/result6.jpg)

进一步研究发现，原来验证集里的很多图片中同时有几列火车，有好几个标注，我的模型通常只召回一个标注，所以召回率比较低了。例如下面图中来自验证集的两张图片，左边有2个标注，右边有3个标注，但模型都只召回了一个，所以召回率比较低：

![结果图片](img/yolo/low_recall.jpg)
