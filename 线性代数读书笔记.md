...
根据coursera上的一门英国帝国大学的线性代数课程：

1. 向量是一种在空间里到处移动的对象，有方向、长度这样的属性，定义在其上面的有标量乘、点积、加法、线性组合等等运算
...
其中有红、绿、黑三个典型向量，红色和绿色向量是两个特征向量，映射后方向不变，只发生了拉伸，拉伸的倍数等于特征值；黑色的向量被拉伸和旋转了。
