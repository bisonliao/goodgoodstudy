...
2. 三个特征向量，分别指示的方向符合预期：y=-x方向（数据标准化后就从y=100-x移动到y=-x了）、z轴方向，y=x方向，三个方向是正交的。
3. 按照公式计算出新的代换变量data2，检视它的前5个采样，他们的第三个feature绝对值也比较小，数据几乎散落在一个平面内
4. 对数据进行降维，抛弃第三成分y3，得到data3，将数据投射到二维面屏上
