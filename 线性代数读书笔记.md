...
如何解决矩阵很大无法求协方差矩阵做特征值分解的问题，wiki里给出一个方法：

> Meaning that, if **u**i is an eigenvector of **T**T**T**, then **v**i = **Tu**i is an eigenvector of **S**. If we have a training set of 300 images of 100 × 100 pixels, the matrix **T**T**T** is a 300 × 300 matrix, which is much more manageable than the 10,000 × 10,000 covariance matrix. Notice however that the resulting vectors **v**i are not normalised; if normalisation is required it should be applied as an extra step.
