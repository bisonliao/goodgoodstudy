...
# 初始化 (所有张量在GPU上)
V = zeros(|S|, device='cuda')          # 价值函数，(|S|,)形状的零张量
π = randint(0, |A|, size=(|S|,), device='cuda')  # 随机策略， (|S|,)形状的张量，每个元素值随机在A的范围内

# 将P和R转移到GPU内存，值都提前计算好填充好
P = cuda.to_gpu(P)  # 形状 [|S|, |A|, |S|]，三维张量，每个元素表示一个概率值，表示P(s'|s,a)
R = cuda.to_gpu(R)  # 形状 [|S|, |A|],二维张量，表示回报 R(s,a)
...
while not converged:
    ...
    for _ in range(max_eval_iters):
        ...
        # 2. 广播选取 P(s, π(s), s') 和 R(s, π(s))
        P_selected = torch.gather(P, 1, a_indices.expand(-1, -1, |S|)).squeeze(1)  # [|S|, |S|]
        R_selected = torch.gather(R, 1, π.unsqueeze(1)).squeeze(1)  # [|S|]
        # 3. 并行计算 V(s) = Σ P * (R + γV_old)
        V = (P_selected * (R_selected.unsqueeze(1) + γ * V_old.unsqueeze(0))).sum(dim=1)  # 广播乘法+求和
        if (V - V_old).abs().max() < δ: # 相减后的张量的每个元素的绝对值都小于epsilon
            break

    # --- 策略改进 (并行化argmax) ---
    # 1. 计算所有动作的Q值 [|S|, |A|]
    Q = (P * (R.unsqueeze(2) + γ * V.unsqueeze(0).unsqueeze(0))).sum(dim=2)  # 广播 [|S|,|A|,|S|] * [|S|,|A|,1]
    # 2. 并行argmax得到新策略
    π_new = Q.argmax(dim=1)  # [|S|]
    if (π == π_new).all():
        break
    π = π_new        
```
...
while not converged:
    V_old = V.clone()
    # 1. 并行计算所有Q值 [|S|, |A|]
    Q = (P * (R.unsqueeze(2) + γ * V_old.unsqueeze(0).unsqueeze(0))).sum(dim=2)  # 广播乘法+求和
    # 2. 取每状态的最大Q值更新V
    V = Q.max(dim=1).values  # 并行规约
    if (V - V_old).abs().max() < δ:
        break

# 提取策略 (同上Policy Improvement)
...
def value_iteration(P:torch.Tensor, R:torch.Tensor, gamma:float):

    V = torch.zeros((state_num), dtype=torch.float32, device=device)
    pi = torch.randint(-5, 6, (state_num), dtype=torch.int32, device=device)
    P = P.to(device)
    R = R.to(device)
    epison = 0.01
    while True:
        V_old = V.clone()
        tmp = gamma*(V_old.unsqueeze(0).unsqueeze(0))
        tmp = tmp + R.unsqueeze(2)
        Q = torch.mm(P, tmp)
        Q = Q.sum(dim=2)
        V = Q.max(dim=1)
        diff = (V - V.old) # type:torch.Tensor
        diff = diff.abs()
        if (diff < epison).all():
            break

    tmp = gamma * (V.unsqueeze(0).unsqueeze(0))
    tmp = tmp + R.unsqueeze(2)
    Q = torch.mm(P, tmp)
    Q = Q.sum(dim=2)
    Pi = Q.argmax(dim=1)
...
def print_value(value):
    for num1 in range(20 + 1):
        for num2 in range(20 + 1):
            state = car_num_to_state(num1, num2)
            print(f"{value[state].item():.2f} ")
        print("")
def prepare_arguments():
    P = torch.zeros((state_num, action_num, state_num), dtype=torch.float32, device=device)
    R = torch.zeros((state_num, action_num), dtype=torch.int32, device=device)
