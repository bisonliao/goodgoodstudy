...
class PPGAgent:
    ...
    def auxiliary_phase_update(self, states, returns):
        """完整的辅助阶段更新函数（包含内部缓存old_probs）"""
        total_loss = 0
        batch_count = 0

        # 1. 在minibatch循环前，预先计算所有old_probs并缓存
        with torch.no_grad():
            old_probs_all = []
            # 分段计算避免内存爆炸
        ...
        for start in range(0, len(states), self.batch_size):
            ...
            aux_loss = F.mse_loss(aux_values.squeeze(-1), returns[start:end])
            ...
            # 2.3 联合优化
            loss = aux_loss + self.beta_clone * kl_loss
        ...
        return total_loss / batch_count  # 返回平均loss
        ...
        while episode < max_episodes:
            ...
            if len(self.buffer.trajectories) > 0:
                ...
                for _ in range(self.e_aux):
                    jl = self.auxiliary_phase_update(states, returns)
