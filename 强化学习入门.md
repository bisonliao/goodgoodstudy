...
```python
'''
openai gym提供的可用于强化学习的游戏，是否支持给RL提供感官像素级的高维输入吗？
如果可以，请以CartPole游戏为例，用Actor-Critic方法训练一个agent，可以很好的玩CartPole游戏，要求：
...
# 光流就是图像中物体的“移动轨迹”或者“像素的移动方向”
def compute_optical_flow_stack(frames: torch.Tensor) -> torch.Tensor:
    """
    输入:
        frames: Tensor, shape (batch_size, 10, 84, 84), 单通道图像帧
    输出:
        flows: Tensor, shape (batch_size, 18, 84, 84), 每两帧计算一个光流 (dx, dy)
    """
    chn = frames.shape[1] # 可能是10
    batch_size = frames.shape[0]
    assert frames.dim() == 4
    flows = []

    frames = frames.cpu().numpy()  # 转为 numpy


    for b in range(batch_size):
        flow_channels = []
        for i in range(chn-1):  # 共9对连续帧
            prev = frames[b, i]
            next = frames[b, i+1]
            # 使用 OpenCV 的 Farneback 光流法（GPU 不支持 Farneback，CPU计算）
            flow = cv2.calcOpticalFlowFarneback(prev, next, None,
                                                pyr_scale=0.5, levels=3, winsize=15,
                                                iterations=3, poly_n=5, poly_sigma=1.2, flags=0)
            # flow shape: (84, 84, 2)
            flow_channels.append(flow[..., 0])  # dx
            flow_channels.append(flow[..., 1])  # dy
        flows.append(np.stack(flow_channels, axis=0))  # shape: (18, 84, 84)

    flows = torch.tensor(np.stack(flows, axis=0), dtype=torch.float32)  # shape: (B, 18, 84, 84)
    return flows.cuda()  # 返回 CUDA 张量


def visualize_and_save_optical_flow(frames, flows, save_dir="./optical_flow"):
    """
    将连续帧和它们之间的光流信息合并成一张大图保存（9行 × 3列：frame1, frame2, optical flow）

    参数：
        frames: shape [1, 10, 84, 84]，像素范围 [0, 1]
        flows: shape [1, 18, 84, 84]，每两个通道一组（dx, dy），表示两个相邻帧的光流
        save_dir: 保存图片的目录
    """
    chn = frames.shape[1]  # 可能是10
    batch_size = frames.shape[0]
    ...
    frames = frames.squeeze(0).cpu().numpy()  # [10, 84, 84]
    flows = flows.squeeze(0).cpu().numpy()    # [18, 84, 84]
    ...
    for i in range(num_pairs):
        ...
        hsv[..., 1] = 255
        hsv[..., 2] = np.clip(mag * 255.0 / (np.max(mag) + 1e-5), 0, 255)
        ...
        axes[i, 2].imshow(flow_bgr)
        axes[i, 2].set_title('Optical Flow')
    ...
    save_path = os.path.join(save_dir, f'optical_flow_grid_{timestamp}.png')
    plt.savefig(save_path)
    plt.close(fig)

class Preprocessor:
    """
    图像预处理工具类，将原始像素转换为适合网络的输入
    """

    def __init__(self, stack_sz = 10, frame_size=84):
        self.frame_size = frame_size
        self.frame_buffer = None
        self.stack_size = stack_sz

    # 修改预处理，突出杆的角度变化
    def process_frame(self, frame):
        gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)

        # 高对比度处理（CartPole需要突出杆的边缘）
    ...
    def stack_frames(self, frame, is_new_episode=False):
        ...
        if is_new_episode or self.frame_buffer is None:
            # 如果是新回合，用相同帧初始化堆叠
            self.frame_buffer = np.stack([processed] * self.stack_size, axis=0)
        else:
            # 否则滚动替换最老的帧
            self.frame_buffer = np.roll(self.frame_buffer, -1, axis=0)
            self.frame_buffer[-1] = processed
...
class ActorCriticCNN(nn.Module):
    """
    基于CNN的Actor-Critic网络，处理像素输入 + 光流
    """
    def __init__(self, input_channels, output_dim, hidden_dim=512):
        super(ActorCriticCNN, self).__init__()

        # 总输入通道数 = 原始图像 + 光流
        total_input_channels = input_channels # + (input_channels-1)*2

        # 共享的CNN特征提取器
        self.feature_extractor = nn.Sequential(
            nn.Conv2d(total_input_channels, 64, kernel_size=8, stride=4),
            nn.ReLU(),
        ...
        # 测试特征维度
        test_input = torch.zeros(1, total_input_channels, 84, 84)
        with torch.no_grad():
            test_output = self.feature_extractor(test_input)
        feature_dim = test_output.shape[1]

        # 修改Actor头结构：添加残差连接 + 层标准化
        self.actor = nn.Sequential(
            nn.Linear(feature_dim, 256),
            nn.LayerNorm(256),  # 稳定梯度流动
            nn.LeakyReLU(0.1),  # 避免ReLU死亡神经元
        ...
        # 初始化最后一层为接近均匀分布
        nn.init.orthogonal_(self.actor[-2].weight, gain=1e-2)
        nn.init.orthogonal_(self.critic[-2].weight, gain=1e-2)

    def forward(self, x, save_pic=False):
        """
        x: 原始图像输入, shape: (B, 10, 84, 84)
        """
        '''flow = compute_optical_flow_stack(x)  # shape: (B, 18, 84, 84)
        x_combined = torch.cat([x, flow], dim=1)  # shape: (B, 28, 84, 84)
        if x.sum().item() == (84*84*10*x.shape[0]): #为全白
            print("x is totally white!!")
        if save_pic:
            visualize_and_save_optical_flow(x[0:1], flow[0:1])'''
        x_combined = x

        features = self.feature_extractor(x_combined)
        action_probs = F.softmax(self.actor(features), dim=-1)
        state_value = self.critic(features)
        return action_probs, state_value

class ActorCriticPixelAgent:
    """
    基于像素输入的Actor-Critic算法实现（使用TD更新）
    """

    def __init__(self, env, device='cuda', gamma=0.95, lr=0.0001,
        ...
        # 初始化网络 (输入是堆叠的4帧84x84灰度图)
        input_channels = 4  # 堆叠的帧数
        # 初始化帧预处理工具
        self.preprocessor = Preprocessor(stack_sz=input_channels, frame_size=84)
        output_dim = env.action_space.n
        self.network = ActorCriticCNN(input_channels, output_dim).to(self.device)
        ...
        # TensorBoard 日志
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.writer = SummaryWriter(f"{tb_log_dir}/cartpole_pixel_ac_td_{timestamp}")
    ...
    def get_state(self, render_output):
        """
        从渲染输出获取状态表示
        """
        # 将渲染输出转换为numpy数组
        frame = np.array(render_output)

        # 预处理并堆叠帧
        state = self.preprocessor.stack_frames(frame)

        # 转换为PyTorch张量并添加批次维度
        state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
        return state_tensor

    def select_action(self, state_tensor, step=None, epsilon=None):
        """
        根据当前策略选择动作
        """
        save_pic = False
        if not (step is None) and step == 10:
            save_pic = True
        action_probs, state_value = self.network(state_tensor, save_pic)
        ...
        if epsilon is not None:
            if random.random() < epsilon:
                action = torch.randint_like(action, 0, 2, device=action.device)
    ...
    def update(self, log_probs, values, rewards, dones, next_values, step_cnt):
        """
        使用TD方法更新网络参数
        """
        # 计算TD目标
        ...
        for reward, done, next_value in zip(rewards, dones, next_values):
            if done:
                ...
                targets.append(reward + self.gamma * next_value.item())  # 使用.item()避免构建计算图

        targets = torch.tensor(targets).float().to(self.device)
        values = torch.cat(values).to(self.device)

        #计算熵
        dist = Categorical(torch.cat([lp.exp() for lp in log_probs]))  # 重建分布
        entropy = dist.entropy().mean()  # 计算熵
        self.writer.add_scalar("steps/entropy", entropy, step_cnt)
        ...
        critic_loss = F.mse_loss(values, targets.unsqueeze(-1))
        total_loss = actor_loss + critic_loss - 0.01*entropy
        self.writer.add_scalar("steps/loss_ratio", actor_loss.item()/critic_loss.item(), step_cnt)
        # 反向传播和优化
        ...
        for name, param in self.network.named_parameters():
            if param.grad is None:
                self.writer.add_scalar(f"steps/{name}", 0, step_cnt)
            else:
                self.writer.add_scalar(f"steps/{name}", param.grad.mean().item(), step_cnt)

        self.optimizer.step()

        return actor_loss.item(), critic_loss.item(), total_loss.item()

    def train(self, max_episodes=5000, max_steps=500,
        ...
        for episode in range(1, max_episodes + 1):
            ...
            state_tensor = self.get_state(render_output)
            self.preprocessor.stack_frames(render_output, is_new_episode=True)
            ...
            rewards = []
            dones = []
            next_values = []  # 存储下一状态的价值

            for step in range(max_steps):
                step_cnt += 1
                # 选择动作
                action, log_prob, value = self.select_action(state_tensor, step, epsilon= epsilon)
                self.writer.add_scalar("steps/action", action, step_cnt)
                _, reward, done, _, _ = self.env.step(action)
                render_output = self.env.render()
                #下面三行代码验证了 多次调用render并不能获得有差异的画面用来推导动力学参数
                '''
                time.sleep(0.05)
                render_output2 = self.env.render()
                assert (torch.tensor(render_output) == torch.tensor(render_output2)).long().sum() == 720000, "render() returns diff."
                '''

                next_state_tensor = self.get_state(render_output)
            ...
            # 使用TD方法更新网络
            actor_loss, critic_loss, total_loss = self.update(log_probs, values, rewards, dones, next_values, step_cnt)
            ...
            # 记录到TensorBoard
            self.writer.add_scalar('episode/episode_reward', episode_reward, episode)
            self.writer.add_scalar('episode/avg_reward_100',
                                   np.mean(self.episode_rewards[-100:]), episode)
            self.writer.add_scalar('episode/actor_loss', actor_loss, episode)
            self.writer.add_scalar('episode/critic_loss', critic_loss, episode)
            self.writer.add_scalar('episode/total_loss', total_loss, episode)
            self.writer.add_scalar('episode/episode_length', episode_length, episode)
        ...
        # 训练结束，保存最终模型
        self.save_checkpoint(max_episodes, is_final=True)
        self.writer.close()

    def evaluate(self, checkpoint_path=None, num_episodes=5, render=True):
        """
        评估模型性能
        """
        if checkpoint_path:
        ...
        if render:
            # 如果需要渲染，创建带渲染的env包装器
            eval_env = wrappers.Monitor(self.env, './pixel_videos_td/', force=True)
        else:
            eval_env = self.env
        ...
        for episode in range(1, num_episodes + 1):
            eval_env.reset()
            render_output = eval_env.render(mode='rgb_array')
            state_tensor = self.get_state(render_output)
            self.preprocessor.stack_frames(render_output, is_new_episode=True)
    ...
    def save_checkpoint(self, episode, is_best=False, is_final=False):
        """
        保存模型检查点
        """
        checkpoint = {
            ...
            'preprocessor_state': {
                'frame_buffer': self.preprocessor.frame_buffer
            }
        }
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        if 'preprocessor_state' in checkpoint:
            self.preprocessor.frame_buffer = checkpoint['preprocessor_state']['frame_buffer']
...
def main(args):
    if args == 'train':
        ...
        # 初始化智能体
        agent = ActorCriticPixelAgent(env)
        agent.train()
    else:
        # 评估模式
        env = gym.make('CartPole-v1', render_mode="human")
        # 初始化智能体
        agent = ActorCriticPixelAgent(env)
        agent.evaluate(checkpoint_path="checkpoints/raw_input.pth",
                       num_episodes=5,
                       render=True)
...
class ReplayBuffer:
    ...
    兼容原接口的优先经验回放实现
    新增参数说明：
    - alpha: 优先级程度 (0=均匀采样, 1=完全按优先级)
    - beta: 重要性采样补偿 (0=无补偿, 1=完全补偿)
    - epsilon: 最小优先级（避免零概率）
    """
    ...
    def push(self, *args):
        ...
        # 新数据赋予当前最大优先级
        max_prio = self.priorities.max() if self._size > 0 else 1.0
        self.priorities[self._pos] = max_prio

        self._pos = (self._pos + 1) % self.capacity
        self._size = min(self._size + 1, self.capacity)

    def sample(self, batch_size: int) -> List[Transition]:
        """优先采样批次数据（返回类型与原接口完全一致）"""
        if self._size < batch_size:
            warnings.warn(f"Buffer only has {self._size} samples, returning all")
        ...
        self._last_probs = probs[indices]
    ...
    def update_priorities(self, td_errors: np.ndarray):
        ...
        新增方法：更新最后采样样本的优先级
        参数：
        - td_errors: 形状为(batch_size,)的TD误差数组
        """
        if not hasattr(self, '_last_indices'):
            raise RuntimeError("Must call sample() before update_priorities()")

        # 计算新优先级
        new_priorities = np.abs(td_errors) + self.epsilon
        new_priorities **= self.alpha

        # 更新优先级数组
        for idx, prio in zip(self._last_indices, new_priorities):
    ...
    def get_importance_weights(self) -> np.ndarray:
        """获取最后采样批次的重要性权重（用于损失函数）"""
        if not hasattr(self, '_last_probs'):
            return np.ones(len(self.buffer))

        # 计算权重 (batch_size,)
        weights = (self._size * self._last_probs) ** (-self.beta)
...
class DQNAgent:
    ...
    def __init__(self, env: gym.Env, stack_size: int = 4, buffer_capacity: int = 20000,
                 ...
                 final_epsilon: float = 0.05, epsilon_decay: float = 0.9999):
    ...
    def update_model(self) -> float:
       ...
       # PER加权损失计算
        weights = torch.tensor(self.memory.get_importance_weights()).to(device)
