...
Q(s, a)函数，即action-value函数，能返回状态s下采取行动a的未来总的奖励期望值。agent可以根据Q(s, a)函数，在每个状态下采取奖励期望值最大的行动，持续进行下去。

DQN就是要训练出一个深度神经网络，拟合Q(s, a)函数。
...
4. **目标网络（Target Network）**：用一个 **滞后的 Q 网络**（QtargetQ_{\text{target}}）计算目标 Q 值，减少训练不稳定性。
...
checkpoint_path = "dqn_checkpoint.pth"  # 模型保存路径
