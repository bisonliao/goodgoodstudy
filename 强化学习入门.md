...
1. 棋盘大小改为5x5
2. 连成3个就算赢
3. 让RL程序执黑，和人肉智能对弈（black_step()函数背后会有白子执行人肉智能），

简化后训练会有效果，慢慢的黑棋的赢率会达到53%。与人类对弈，人类也会明显发现训练后的模型会比训练前聪明。
...
class PolicyNet(nn.Module):
    ...
    def forward(self, state):
        noise = torch.randn_like(state, device=device) / 1000000.0
        ...
        # 通过 CNN 提取特征
        x = F.relu(self.conv1(state+noise))
