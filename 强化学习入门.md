...
def value_iteration(P: torch.Tensor, R: torch.Tensor, gamma: float, epsilon: float = 1e-5):
    """
    PyTorch CUDA-accelerated value iteration for MDPs.
    ...
    Returns:
        Tuple[torch.Tensor, torch.Tensor]: Optimal policy (S,) and value function (S,).
    """
    ...
    while True:
        ...
        # Update V and check convergence
        V = Q.max(dim=1).values
        delta = (V - V_old).abs().max().item()

        if delta < epsilon:
            break
        print(f"Delta#{iter_num} = {delta:.6f}")

    # Extract optimal policy
    Q_final = R + gamma * torch.einsum('sas,s->sa', P, V)
    pi = Q_final.argmax(dim=1)
