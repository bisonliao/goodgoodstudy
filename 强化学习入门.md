...
妈的，探索的epsilon不能乱调，改回来：0.05 / 0.99重新跑！



一句话：学习到了一定的策略，比随机策略有明显提升，但相比低维结构化的状态输入的收敛后的DQN，差距太大

如下图红色线条所示：6千个回合后，每个回合的total_reward均值在80~100之间，但是波动很大，低的50以下，高的有270多的。

使用最高total_reward的checkpoint进行evaluate，10个回合下来平均分在72分。

蓝色线条是均匀随机策略的表现，平均total_reward在20左右，高的能到80左右。
...
结论：效果不好，回合reward和（total_reward） 上升缓慢、方差（波动）巨大、loss值一直在增大
...
###### 改进二：提取角度信息并据此对reward整形

结论：效果不好，回合reward和（total_reward） 上升缓慢、方差（波动）巨大、loss值一直在增大

![image-20250411144141303](img/RL/image-20250411144141303.png)
    ...
    def train(self, num_episodes: int, checkpoint_interval: int = 500,
        ...
        for episode in range(start_episode, num_episodes):
            ...
            while not done:
                ...
                # 存储transition
                self.memory.push(state['frames'], state['angles'], action, next_state['frames'], next_state['angles'], shaped_reward, done)
...
###### 改进三：提取角度信息只用于对reward整形，保持DQN网络简单
