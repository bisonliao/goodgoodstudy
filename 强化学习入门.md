...
def compute_optical_flow_stack(frames: torch.Tensor) -> torch.Tensor:
    ...
    """
    assert frames.dim() == 4 and frames.shape[1] == 10, "Expect input shape: (B, 10, 84, 84)"
    flows = []
...
def visualize_frames_and_optical_flow(frames, flows):
    """
    显示连续帧和它们之间的光流信息
    ...
    for i in range(num_pairs):
        ...
        hsv[..., 2] = np.clip(mag * 255.0 / (np.max(mag) + 1e-5), 0, 255)  # Value
...
class Preprocessor:
    ...
    def stack_frames(self, frame, is_new_episode=False):
        ...
        if is_new_episode or self.frame_buffer is None:
            # 如果是新回合，用相同帧初始化堆叠
            self.frame_buffer = np.stack([processed] * 10, axis=0)
        else:
            # 否则滚动替换最老的帧
...
class ActorCriticCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        if x[0].sum().item() != (84*84*10): #不为全白
            visualize_frames_and_optical_flow(x, flow)
...
class ActorCriticPixelAgent:
    ...
    def __init__(self, env, device='cuda', gamma=0.99, lr=0.0001,
        ...
        # 初始化帧预处理工具
        self.preprocessor = Preprocessor(frame_size=84)

        # 初始化网络 (输入是堆叠的4帧84x84灰度图)
        input_channels = 10  # 堆叠的帧数
    ...
    def update(self, log_probs, values, rewards, dones, next_values):
        """
        使用TD方法更新网络参数
        """
        # 计算TD目标
    ...
    def train(self, max_episodes=5000, max_steps=500,
        ...
        for episode in range(1, max_episodes + 1):
            ...
            actor_loss, critic_loss, total_loss = self.update(
                log_probs, values, rewards, dones, next_values)
            ...
            # 记录到TensorBoard
            self.writer.add_scalar('pixels/episode_reward', episode_reward, episode)
            self.writer.add_scalar('pixels/avg_reward_100',
                                   np.mean(self.episode_rewards[-100:]), episode)
            self.writer.add_scalar('pixels/actor_loss', actor_loss, episode)
            self.writer.add_scalar('pixels/critic_loss', critic_loss, episode)
            self.writer.add_scalar('pixels/total_loss', total_loss, episode)
            self.writer.add_scalar('pixels/episode_length', episode_length, episode)
