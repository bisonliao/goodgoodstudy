...
'''
todo:
1、把过去7步的state也作为7个通道
2、reward差异不能太大，归一化的时候可能会导致掩盖前面的reward效果。AlphaGo是前面一直为0，终局是+-1
'''
...
2. 五子棋返回的reward更简单，没有分出胜负之前，reward都是0，胜出胜负就是+-1
...
class Gomoku:
    ...
    def step(self, role, x, y):
        ...
        if self.check_win(x, y, role):
            ...
            reward = 0 # self.evaluate_move(x, y, role)
        self.render()
        return self.board.copy(), reward, self.game_over
...
class PolicyNet(nn.Module):
    ...
    def forward(self, state):
        ...
        mask = (mask.view(batch_size, 9, 9) == 0).float()
        mask = mask.view(batch_size, 81)
        ...
        x = self.policy_out(x)  # 输出 shape: [batch, 1, 9, 9]

        # 展平成 [batch, 81] 作为 logits
        x = x.view(batch_size, 81)
...
def select_action(policy_net, state):
    ...
    state_tensor = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(0)
    state_tensor = reorganize_state(state_tensor)
    policy_net.eval()
    with torch.no_grad():
        action_probs = policy_net(state_tensor).squeeze().detach().cpu().numpy()  # 转到CPU计算
    policy_net.train()
...
def reorganize_state(states: torch.Tensor):
    # states本身的shape：batchsz, 1, 9, 9
    batchsz = len(states)
    state_tensor = torch.zeros(batchsz, HISTORY_LEN * 2, 9, 9)
