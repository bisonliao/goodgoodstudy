...
3. nsenter，进入到指定进程的某些命名空间。例如 nsenter -t 1630392 -n 表示进入到进程pid为1630392的network namespace。常用这种方式进入指定container的网络名空间
...
利用上面6那里的创建的docker image启动两个容器，作为RS：

```shell
docker run -d --rm --name=calc1 ccr.ccs.tencentyun.com/tsf_2202020202/calc
docker run -d --rm --name=calc2 ccr.ccs.tencentyun.com/tsf_2202020202/calc
```

可以发现这两个容器的ip分别是172.17.0.3和172.17.0.4。
...
而且，很有意思的是，ipvs还可以直接使用eth0的本机真实ip作为vs，例如本机实际ip是172.19.16.7：
...
那这样就可以在使用haproxy的地方用ipvs来替代了，应该性能更好。
...
肯定不是这样：kube-proxy监听dummy网卡上，业务pod发往service cluster ip的报文都会被kube-proxy截获，然后按一定的规则进行负载均衡请求背后的endpoint。因为ipvs模式直接工作在内核态，并不在应用态监听端口。

dummy网卡的作用，主要是把pod业务外发的报文转化为本机输入的报文，从而进入和应用iptables相关规则。（详细的我也没有看太懂）
...
//from https://stackoverflow.com/questions/14478167/bind-socket-to-network-interface
```

## 7.3 k8s_proxy的 ipvs模式的原理
