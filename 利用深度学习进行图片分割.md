...
deconvolution层本质上还是convolution层，并非信号处理学科中的deconvolution的概念。

准确的名字应该叫转置卷积层，来自于深度学习框架在计算卷积的时候，转换为等价矩阵运算（分别求得输入图像和卷积核的Toeplitz矩阵，然后矩阵相乘）。

例如下面upscore8_bison这个deconvolution层，用工具可查看到它的参数的尺寸是：(2, 21, 16, 16)，注意区别于输出或者输入blob的尺寸，即16 X 16 X 21的三维卷积核，这样的卷积核有2个。

![这里有张图片](F:/GitHub/goodgoodstudy/img/FCN/deconv2.jpg)
