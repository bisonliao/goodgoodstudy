...
![这里有张图片](img/FCN/two_class5.jpg)

训练集数据的准备应该还可以改进，有助于提升准确率：

1. 把上面飞机、餐盘这样的图片也加入训练集，其标注为全黑的一张图片（每个像素分类都为0）
2. 对现有的图片做augment后丰富训练集，例如使用python的Augmentor库可以很方便实现

后面有时间可以试一下。

[Augmentor的python脚本示例](code/fcn/augmentor.py)

### 3.3 2019.3.18 尝试image augment来提升准确率

简单的在原来带人体分割信息的训练集上做一些image augment：左右翻转、椒盐噪声、模糊，且在3.2小节得到的模型上进一步finetune，准确率提升有限，在88%左右摆动。

测试情况显示：对于图片中有人体的部分，分割还比较准确，对于没有人体的图片，假阳性率有点高：
