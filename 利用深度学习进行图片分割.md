...
deconvolution层本质上还是convolution层，与信号处理学科中的deconvolution的概念是不是一致的？

准确的名字应该叫转置卷积层，来自于深度学习框架在计算卷积的时候，转换为等价的矩阵相乘。而deconvolution相当于再乘以卷积核对应的Toeplitz矩阵的转置，达到反运算的目的。

例如下面upscore8_bison这个deconvolution层，用工具可查看到它的参数的尺寸是：(2, 21, 16, 16)，注意区别于输出或者输入blob的尺寸，即16 X 16 X 21的三维卷积核，这样的卷积核有2个。

![这里有张图片](img/FCN/deconv2.jpg)

几个参考外链：

[Toeplitz矩阵](https://en.wikipedia.org/wiki/Toeplitz_matrix#Discrete_convolution)

[信号处理学科中的deconvolution](https://en.wikipedia.org/wiki/Deconvolution)

记号：卷积操作的参数分别有**k**ernel size / **p**ad / **s**tride / **i**nput size/ **o**utput size

deconvolution层（参数为k‘、p’、s‘、i'、o'）与之对应的convolution层（参数为k、p、s、i、o），**某些情况下**参数关系为：

```
k' = k， s' = s， p'=k-p-1， i' = o, o'= i
```

举几个例子如下：

![这里有张图片](img/FCN/deconv1.jpg)
...
有点糊涂了：stride >1 的时候，输入的像素之间会被pad？  这似乎和卷积层不一样？
