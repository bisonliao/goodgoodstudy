...
由于官方作者的工程能力和责任心都很有限，代码和文档（如果也叫文档的话）一塌糊涂，以至于csdn上大量的博客写怎么怎么搞定这件事情的，这些作者通常会悲催的弄上两三个星期。
...
1. python真的很强大，同样是处理一个很大的json文件（来自coco数据集），我一开始用jsonc库（号称最好用的c语言下的json库），加载文件失败；改用java语言（我会的语言多啊），提示虚拟机栈空间不够，调整虚拟机参数也没有解决。 python轻松搞定
