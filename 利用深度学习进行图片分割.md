...
这个也可以解释：用来finetune的基础模型可能是一个来自大量数据集、出自专家之手训练出来的模型，分割效果已经非常好（实际调用验证也确实如此），而我在此基础上使用少量的数据集（区区3万张图片，可能还只是原来数据集的一个子集），想要从21分类训练出一个2分类模型，其实只是对原有的模型做了很小的调整，这个调整可能还是更糟糕的。凭什么改天换地呢？
