...
1. 修改upscore8的num_output为2，修改名字（那就没法加载finetune模型的参数了），且filler为xavier，尝试训练，
2. 训练图片仅使用category为人体的3万张图片，还是每个batch 4张，迭代3万次。尝试训练出一个人体分割的模型，比较有实际价值

准确率由51%（完全随机瞎蒙）提升到60%（稍微准确了一点点）：

![这里有张图片](img/FCN/two_class.jpg)

从测试结果也可以看到，就是一堆随机的0,1里有一点点轮廓的样子：

![这里有张图片](img/FCN/two_class2.jpg)

这个网络有几十层，只有一个层是没有底子的开始训练，用了3万丈图片花了5个多小时，准确率只提升了10个点。可能我的姿势不太对，但一定程度上反映图片分割的训练是开销很大的。
